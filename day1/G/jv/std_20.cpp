...
int n,q;
const int N=2e5+5,d=24;
ull a[N],tag[4*N],f[100],g[100],c[100][100];
void precalc(){
    //compute the polynomial x(x+1)...(x+23) modulo 2^32;
    f[1]=1;
    rep(i,1,d-1){
        rep(j,1,d)g[j]=f[j-1]+i*f[j];
        rep(j,1,d)f[j]=g[j];
    }
    rep(i,0,d)c[i][0]=1;
...
poly mul(poly x,poly y){
    ull coef[2*d];memset(coef,0,sizeof(coef));
    rep(i,0,d-1)rep(j,0,d-1)coef[i+j]+=x.coef[i]*y.coef[j];
    for(int i=2*d-2;i>=d;i--){
        rep(j,1,d)coef[i-j]-=coef[i]*f[d-j];coef[i]=0;
    }
    poly z;
