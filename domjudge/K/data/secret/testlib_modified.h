...
struct InStream {
    ...
    void readStrictStringTo(std::string &result, char end_char = '\n');
    ...
    std::string readStrictString(const std::string &ptrn, std::size_t minlen, size_t maxlen, char end_char = '\n', const std::string &variableName = "");
    ...
    void readStrictStringTo(std::string &result, const pattern &p, std::size_t minlen, std::size_t maxlen, char end_char = '\n', const std::string &variableName = "");

    /* The same as "readLine()/readString()", but ensures that line matches to the given pattern. */
    void readStringTo(std::string &result, const std::string &ptrn, const std::string &variableName = "");

    void readStrictStringTo(std::string &result, const std::string &ptrn, std::size_t minlen, std::size_t maxlen, char end_char = '\n', const std::string &variableName = "");
...
void InStream::readStrictStringTo(std::string &result, char end_char) {
    if (NULL == reader)
        quit(_pe, "Expected line");
    ...
    for (;;) {
        ...
        if (cur == LF || cur == EOFC || cur == end_char)
            break;
...
void InStream::readStrictStringTo(std::string &result, const pattern &p, std::size_t minlen, std::size_t maxlen, char end_char, const std::string &variableName) {
    readStrictStringTo(result, end_char);
     if (!p.matches(result)) {
        if (readManyIteration == NO_INDEX) {
            if (variableName.empty())
                quit(_wa, ("Line \"" + __testlib_part(result) + "\" doesn't correspond to pattern \"" + p.src() +
    ...
    if (result.length() < minlen || result.length() > maxlen) {
        if (variableName.empty())
            quit(_wa, ("Strict String " + vtos(result) + " 's length violates the range [" + vtos(minlen) + ", " + vtos(maxlen) +
                        "]").c_str());
        else
            quit(_wa,
                    ("Strict String parameter [name=" + std::string(variableName) + "] equals to " + vtos(result) +
                    ", length violates the range [" + vtos(minlen) + ", " + vtos(maxlen) + "]").c_str());
    }
}
...
void InStream::readStrictStringTo(std::string &result, const std::string &ptrn, std::size_t minlen, std::size_t maxlen, char end_char, const std::string &variableName) {
    readStrictStringTo(result, pattern(ptrn), minlen, maxlen, end_char, variableName);
}
...
std::string InStream::readStrictString(const std::string &ptrn, std::size_t minlen, std::size_t maxlen, char end_char, const std::string &variableName) {
    readStrictStringTo(_tmpReadToken, ptrn, minlen, maxlen, end_char, variableName);
    return _tmpReadToken;
}
